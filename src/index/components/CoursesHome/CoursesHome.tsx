...
//   const [sidebarLimit, setSidebarLimit] = useState(6);
...
//                 {/* SIDEBAR PAGINATION */}
//                 {coursesByCategory.length > 6 && (
//                   <button
//                     onClick={() =>
//                       setSidebarLimit(
//                         sidebarLimit === 6 ? coursesByCategory.length : 6
//                       )
//                     }
//                     className="text-blue-600 text-sm font-medium mt-2 flex items-center gap-1 px-5 hover:underline"
//                   >
//                     {sidebarLimit === 6 ? "View more categories" : "View less"}
//                   </button>
//                 )}
//               </>
//             )}
...
export default function CoursesHome() {
  ...
  // ✅ FETCH GROUPED API (Single optimized call)
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const res = await fetch("/api/course/fetch-grouped");
  ...
  const getRandomRating = () => {
    return (Math.random() * (5 - 4.6) + 4.6).toFixed(1); // 4.6 → 5.0
  };

  const getRandomCount = () => {
    return Math.floor(Math.random() * (7000 - 6000 + 1)) + 6000; // 6000 → 7000
  };

  // 🔹 COURSE CARD
  const CourseCard = ({ course }: { course: Course }) => {
    const rating = Number(getRandomRating());
    const ratingCount = getRandomCount();
                ...
                {coursesByCategory.slice(0, sidebarLimit).map((cat, idx) => (
                  <div key={cat.name}>
                    ...
                    >
                      {cat.name}
                      <ChevronDownIcon
                        className={`md:hidden transition-transform ${selectedCategoryIdx === idx ? "rotate-180" : ""
                          }`}
                      />
                    </button>
