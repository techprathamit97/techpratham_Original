...
const InvoicesDashboard = () => {
  ...
  // LMS Account creation states
  const [isLMSAccountDialogOpen, setIsLMSAccountDialogOpen] = useState(false);
  const [selectedInvoiceForLMS, setSelectedInvoiceForLMS] = useState<Invoice | null>(null);
  const [lmsLoginId, setLmsLoginId] = useState('');
  const [lmsPassword, setLmsPassword] = useState('');
  const [showLmsPassword, setShowLmsPassword] = useState(false);
  const [lmsAccountCreated, setLmsAccountCreated] = useState(false);
  const [lmsAccountExists, setLmsAccountExists] = useState(false);
  ...
  const handleEditInvoice = (invoice: Invoice) => {
    try {
      ...
      if (invoice.isManual) {
        ...
        window.location.href = `/admin/dashboard/create-manual-invoice?edit=${invoice._id}`;
      } else {
        // For regular invoices, navigate to regular invoice edit page
        window.location.href = `/admin/dashboard/create-invoice?edit=${invoice._id}`;
      }
    } catch (error) {
  ...
  }, [authenticated, statusFilter, salesPersonFilter]);
  ...
  // Filter invoices based on status and sales person
  ...
  const handleOpenLMSAccountDialog = async (invoice: Invoice) => {
    setSelectedInvoiceForLMS(invoice);
    setLmsAccountCreated(false);
    setLmsAccountExists(false);
    setCreatedCredentials(null);
    
    // Check if student account already exists
    try {
      const res = await fetch(`/api/lms/check-student-account?studentId=${invoice.customerDetails.studentId}&email=${invoice.customerDetails.email}`);
      const data = await res.json();
      
      if (data.success && data.exists) {
        // Account already exists, show existing credentials
        setLmsLoginId(data.student.studentId);
        setLmsPassword(''); // Don't show password for security
        setLmsAccountExists(true);
        setLmsAccountCreated(true);
        setCreatedCredentials({
          loginId: data.student.studentId,
          password: '••••••••••••' // Masked password
        });
        toast.info('LMS account already exists for this student');
      } else {
        // No account exists, prepare for new account creation
        setLmsLoginId(invoice.customerDetails.studentId || '');
        setLmsPassword(generateLmsPassword());
        setLmsAccountExists(false);
      }
    } catch (error) {
      console.error('Error checking student account:', error);
      // Fallback to new account creation
      setLmsLoginId(invoice.customerDetails.studentId || '');
      setLmsPassword(generateLmsPassword());
      setLmsAccountExists(false);
      toast.error('Could not check existing account. Creating new account.');
    }
    
    setIsLMSAccountDialogOpen(true);
  };

  const handleCreateLMSAccount = async () => {
    if (!selectedInvoiceForLMS) {
      toast.error('No invoice selected');
      return;
    }

    if (!lmsLoginId || lmsLoginId.length < 3) {
      toast.error('Login ID must be at least 3 characters');
      return;
    }

    if (!lmsPassword || lmsPassword.length < 6) {
    ...
    try {
      ...
      if (res.ok) {
        toast.success('LMS account created successfully!');
        setLmsAccountCreated(true);
        setCreatedCredentials({
          loginId: lmsLoginId,
          password: lmsPassword
        ...
        toast.error(data.error || 'Failed to create LMS account');
      }
    } catch (error) {
      console.error('Error creating LMS account:', error);
      toast.error('Network error. Please try again.');
    }
  };
  ...
  return (
    <React.Fragment>
            ...
            <div className="bg-black p-6 overflow-y-auto h-full flex-1">
              <div className='w-full h-auto flex flex-row items-start justify-between mb-8'>
                ...
                <div className="flex gap-4 items-center">
                  ...
                  {isAdmin && (
                    <>
                      <Link href="/admin/dashboard/create-manual-invoice">
                      ...
                      {/* <Link href="/admin/dashboard/create-simple-invoice">
                          ...
                          <tr key={invoice._id} className="border-b border-zinc-800 hover:bg-zinc-800/50">
                            ...
                            <td className="p-3">
                              <div className="flex gap-1 flex-wrap">
                                ...
                                {/* Only show Pay button if there's pending amount - same as accountant dashboard */}
                                {invoice.pendingAmount > 0 && isAdmin && (
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    className="border-zinc-700 text-zinc-300 hover:bg-zinc-800 text-xs px-2 py-1"
                                ...
                                {/* Create LMS Account Button */}
                                {isAdmin && (
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    className="border-green-700 text-green-400 hover:bg-green-900/20 text-xs px-2 py-1"
                                    onClick={() => handleOpenLMSAccountDialog(invoice)}
          ...
          {selectedPayment && (
            <div className="space-y-4">
              ...
              <div className="text-sm text-zinc-400">
                ...
                <div>This is E-Invoice, Signature Not Required. For any query pls write us accounts@techpratham.com</div>
              </div>

              {/* Thank You */}
              <div className="text-center">
                <div className="text-2xl font-bold text-white">Thank you!</div>
                <div className="text-zinc-400">for joining us</div>
                <div className="text-white font-semibold mt-2">Bharat Sahai</div>
                <div className="text-zinc-400 text-sm">Authorised Signature</div>
              </div>
      ...
      {/* Create LMS Account Dialog */}
      <Dialog open={isLMSAccountDialogOpen} onOpenChange={setIsLMSAccountDialogOpen}>
        <DialogContent className="bg-black border-zinc-800 text-white max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <UserPlus className="h-5 w-5 text-green-400" />
              {lmsAccountExists ? 'Existing LMS Account' : 'Create LMS Account'}
            </DialogTitle>
            <DialogDescription className="text-zinc-400">
              {lmsAccountExists 
                ? `LMS account already exists for ${selectedInvoiceForLMS?.customerDetails.name}`
                : `Create student login credentials for ${selectedInvoiceForLMS?.customerDetails.name}`
              }
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-zinc-900 p-3 rounded-lg">
                <div className="text-sm space-y-1">
                  <div><span className="text-zinc-400">Name:</span> <span className="text-white font-medium">{selectedInvoiceForLMS?.customerDetails.name}</span></div>
                  <div><span className="text-zinc-400">Email:</span> <span className="text-white">{selectedInvoiceForLMS?.customerDetails.email}</span></div>
                  <div><span className="text-zinc-400">Phone:</span> <span className="text-white">{selectedInvoiceForLMS?.customerDetails.phone}</span></div>
                  <div><span className="text-zinc-400">Course:</span> <span className="text-white">{selectedInvoiceForLMS?.courseDetails.title}</span></div>
                </div>
              </div>

              {/* Login ID */}
              <div>
                <Label htmlFor="lms-login-id">Student Login ID *</Label>
                <Input
                  id="lms-login-id"
                  value={lmsLoginId}
                  onChange={(e) => setLmsLoginId(e.target.value)}
                  className="bg-zinc-900 border-zinc-700 text-white"
                  placeholder="Enter unique login ID"
                />
                <div className="text-xs text-zinc-500 mt-1">
                  Min 3 characters. This will be used to login to student dashboard.
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <Input
                      id="lms-password"
                      type={showLmsPassword ? 'text' : 'password'}
                      value={lmsPassword}
                  ...
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    onClick={() => setLmsPassword(generateLmsPassword())}
                    className="px-3 border-zinc-700"
                ...
                <div className="text-xs text-zinc-500 mt-1">
                  Min 6 characters. Click refresh to auto-generate.
                </div>
              </div>

              {/* Login URL Info */}
              <div className="bg-blue-900/20 border border-blue-700 p-3 rounded-lg">
                <div className="text-blue-400 text-sm font-medium">ℹ️ Student Login</div>
                <div className="text-blue-300 text-xs mt-1">
                  Students can login at: <span className="font-mono">/student/login</span>
                </div>
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <div className="bg-green-900/20 border border-green-700 p-4 rounded-lg text-center">
                ...
                <div className="text-green-400 font-medium text-lg">
                  {lmsAccountExists ? 'Account Already Exists!' : 'Account Created Successfully!'}
                </div>
                <div className="text-green-300 text-sm mt-1">
                  {lmsAccountExists 
                    ? 'Student can use these credentials to login'
              ...
              <div className="bg-zinc-900 p-4 rounded-lg space-y-3">
                <div>
                  <Label className="text-zinc-400 text-xs">Student Login ID</Label>
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex-1 bg-black p-2 rounded border border-zinc-700">
                      <span className="text-white font-mono">{createdCredentials?.loginId}</span>
                    </div>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => copyToClipboard(createdCredentials?.loginId || '', 'Login ID')}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex-1 bg-black p-2 rounded border border-zinc-700">
                      <span className="text-white font-mono">{createdCredentials?.password}</span>
                    </div>
                    {!lmsAccountExists && (
                      <Button
                        size="sm"
                  ...
                  {lmsAccountExists && (
                    <div className="text-xs text-yellow-400 mt-1">
                      Password is hidden for security. Student should use their existing password.
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex-1 bg-black p-2 rounded border border-zinc-700">
                      <span className="text-white font-mono text-xs">/student/login</span>
                    </div>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => copyToClipboard('/student/login', 'Login URL')}
                      className="border-zinc-700"
                    >
                      <Copy className="h-4 w-4" />
                    </Button>
              ...
              <div className="bg-yellow-900/20 border border-yellow-700 p-3 rounded-lg">
                <div className="text-yellow-400 text-sm font-medium">⚠️ Important</div>
                <div className="text-yellow-300 text-xs mt-1">
                  {lmsAccountExists 
                    ? 'This account was created previously. Student should use their existing password to login.'
            ...
            {!lmsAccountCreated ? (
              <>
                <Button
                  ...
                  className="border-zinc-700 text-zinc-300 hover:bg-zinc-800"
                  onClick={() => {
                    setIsLMSAccountDialogOpen(false);
                    setLmsLoginId('');
                    setLmsPassword('');
                    setSelectedInvoiceForLMS(null);
                    setLmsAccountExists(false);
                  }}
                >
                  Cancel
                ...
                <Button 
                  variant="manual" 
                  onClick={handleCreateLMSAccount}
                  disabled={!lmsLoginId || !lmsPassword || lmsLoginId.length < 3 || lmsPassword.length < 6}
