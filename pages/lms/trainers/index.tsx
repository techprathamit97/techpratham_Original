...
const TrainersManagement = () => {
  ...
  const [loginIdCheckStatus, setLoginIdCheckStatus] = useState<'idle' | 'checking' | 'available' | 'taken'>('idle');
  ...
  const checkEmailAvailability = async (email: string) => {
    ...
    try {
      const res = await fetch(`/api/lms/trainers/check-email?email=${encodeURIComponent(email)}`);
  ...
  const checkLoginIdAvailability = async (loginId: string) => {
    ...
    try {
      const res = await fetch(`/api/lms/trainers/check-login-id?loginId=${encodeURIComponent(loginId)}`);
      const data = await res.json();
  ...
  const handleLoginIdChange = (loginId: string) => {
    setNewTrainer(prev => ({ ...prev, loginId }));
    
    // Debounce login ID check
    const timeoutId = setTimeout(() => {
  ...
  const handleCreateTrainer = async () => {
    // Validate required fields
    if (!newTrainer.name || !newTrainer.email || !newTrainer.phone || !newTrainer.experience || !newTrainer.loginId || !newTrainer.loginPassword) {
      toast.error('Please fill in all required fields including Login ID and Password');
      return;
    }
    ...
    if (newTrainer.loginId.length < 3) {
    ...
    try {
      ...
      if (res.ok) {
        toast.success('Trainer added successfully with login credentials!');
            ...
            <DialogContent className="bg-gray-800 border-gray-700 max-w-2xl max-h-[80vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gradient-to-r from-green-900/30 to-teal-900/30 border border-green-700/50 rounded-lg p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label className="text-white">Login ID (Unique) *</Label>
                      <div className="relative">
                        <Input
                          value={newTrainer.loginId}
                          onChange={(e) => handleLoginIdChange(e.target.value)}
                          className="bg-gray-700 border-gray-600 text-white pr-10"
                          placeholder="e.g., trainer001"
                        />
                        {loginIdCheckStatus === 'checking' && (
                      ...
                      {loginIdCheckStatus === 'taken' && (
                        <p className="text-red-400 text-xs mt-1">This Login ID is already taken</p>
                      )}
                      {loginIdCheckStatus === 'available' && (
                        <p className="text-green-400 text-xs mt-1">Login ID is available</p>
                      )}
                    </div>
              ...
              <div className="flex justify-end gap-2 mt-4">
                ...
                <Button 
                  ...
                  disabled={!newTrainer.name || !newTrainer.email || !newTrainer.loginId || !newTrainer.loginPassword || emailCheckStatus === 'taken' || emailCheckStatus === 'checking' || loginIdCheckStatus === 'taken' || loginIdCheckStatus === 'checking'}
                >
                  Add Trainer
                </Button>
              </div>
          ...
          {editTrainer && (
            <div className="space-y-4">
              ...
              <div className="bg-gradient-to-r from-green-900/30 to-teal-900/30 border border-green-700/50 rounded-lg p-4 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-white">Login ID (Read-only)</Label>
                    <Input
                      value={editTrainer.loginId || 'Not set'}
                      disabled
                      className="bg-gray-600 border-gray-500 text-gray-300 cursor-not-allowed"
                    />
                    <p className="text-gray-400 text-xs mt-1">Login ID cannot be changed</p>
                  </div>
